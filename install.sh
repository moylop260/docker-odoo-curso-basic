...
apt-get install -y python-pip libxml2-dev libxslt-dev libevent-dev \
    ...
    locales tree sudo bsdmainutils
# bsdmainutils - column command to see csv files
...
npm install -g less
wget https://github.com/wkhtmltopdf/wkhtmltopdf/releases/download/0.12.1/wkhtmltox-0.12.1_linux-trusty-amd64.deb -O /tmp/wk.deb && dpkg -i /tmp/wk.deb
